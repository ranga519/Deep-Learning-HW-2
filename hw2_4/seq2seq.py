...
if __name__ == "__main__":
 ...
 # Convert id2word to a list of tuples with IDs, words, and counts
    table_data = [(key, value, word_counts.get(value, 0)) for key, value in id2word.items()]

# Sort the table data alphabetically by the word column
    sorted_table_data = sorted(table_data, key=lambda x: x[1])

# Print the table
    print(tabulate(sorted_table_data, headers=["ID", "Vocabulary", "Count"]))
    ...
    # initial model
    s2vt = model1(vocab_size=vovab_size, batch_size=BATCH_SIZE)
    print(s2vt)
    #exit(0)
    ...
    # Assuming you have a list named loss_list containing the loss values during training
...
# Showing the plot (optional)
    ...
    print("Testing finished! Output text file saved!")
